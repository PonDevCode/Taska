...
import VisuallyHiddenInput from '../../../components/From/VisuallyHiddenInput'
...
import { updateCardDetailsAPI } from '~/apis/axios'
...
const SidebarItem = styled(Box)(() => {
  ...
  return {
    ...
    backgroundColor: status === 'dark' ? '#2f3542' : '#091e420f',
    padding: '10px',
    borderRadius: '4px',
    '&:hover': {
...
function ActiveCard() {
  ...
  // const [isOpen, setIsOpen] = useState(isShowModalActiveCard)
  // const handleOpenModal = () => setIsOpen(true)
  const handleCloseModal = () => {
    dispacth(clearAndHideCurrentActiveCard())``
  }

  // function dùng chung cho các trường hợp update card (title, des, cm , ....)
  const callApiUpdateCard = async (updateData) => {
    const updateCard = await updateCardDetailsAPI(activeCard._id, updateData)
    // b1 cập nhật lại các card đang active trong model hiện tại
    dispacth(updateCurrentActiveCard(updateCard))
    // b2 cập nhật lại các bản ghi card trong cái activeBoard (nested data)
    dispacth(updateCardInBoard(updateCard))
    return updateCard
  }
  ...
  const onUpdateCardDescription = (des) => {
    callApiUpdateCard({ description: des })
  }
  ...
  // dùng async await ở đây để component con CardActiveSection chờ và nếu thành công thì clear thẻ input comment
