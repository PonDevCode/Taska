...
import { createNewBoardAPI } from '~/apis/axios'
...
/**
 * Bản chất của cái component SidebarCreateBoardModal này sẽ trả về một cái SidebarItem để hiển thị ở màn Board List cho phù hợp giao diện bên đó,
 *  đồng thời nó cũng chứa thêm một cái Modal để xử lý riêng form create board nhé.
 *  Modal là một low-component mà bọn MUI sử dụng bên trong những thứ như Dialog, Drawer, Menu, Popover.
 *  Ở đây  có thể sử dụng Dialog cũng không thành vấn đề gì, nhưng sẽ sử dụng Modal để dễ linh hoạt tùy biến giao diện từ con số 0 cho phù hợp với mọi nhu cầu.
 */
function SidebarCreateBoardModal({isLoad, setIsLoad}) {
    ...
    <>
      ...
      >
        <Box sx={{
          ...
          outline: 0,
          padding: '20px 30px',
          backgroundColor: status === 'dark' ? '#1A2027' : 'white'
        }}>
          <Box sx={{
            position: 'absolute',
            top: '10px',
          ...
          <Box id="modal-modal-description" sx={{ my: 2 }}>
            <form onSubmit={handleSubmit(submitCreateNewBoard)}>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                {/*
                  *  đối với RadioGroup của MUI thì không thể dùng register tương tự TextField được mà phải sử dụng <Controller /> và props "control" của react-hook-form như cách làm dưới đây
