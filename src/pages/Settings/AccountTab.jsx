...
function AccountTab() {
  ...
  const submitChangeGeneralInformation = (data) => {
    ...
    // Gọi API...
    toast.promise(
      dispacth(updateUserAPI({displayName})),
      {pending: 'update....'}
    ).then(res=> {
      // update thành công ko có lỗi thực hiện hành động cần thiết
      if(!res.error){
  ...
  const uploadAvatar = (e) => {
    ...
    // Gọi API...
    toast.promise(
      dispacth(updateUserAPI(reqData)),
      {pending: 'update....'}
    ).then(res=> {
      // update thành công ko có lỗi thực hiện hành động cần thiết
      if(!res.error){
      ...
      // lưu ý có thành công hay không thì cũng phải clear giá trị file input , nếu không thì sẽ không thể chọn cùng 1 file liên tiếp
      e.target.value = ''
    })
  }
